...
Cores = {
    ...
    "verde": "\033[42m",     # Fundo verde para o tabuleiro
    "amarelo": "\033[93m",   # Cor dos números
    "ciano": "\033[96m",
}
...
def mostrar_tabuleiro(tabuleiro):
    limpar_tela()
    print("\n" + Cores["verde"] + "  JOGO DA VELHA - Contra IA" + Cores["reset"])
    print("┌───┬───┬───┐")
    for i in range(3):
        print(f"│ {Cores['vermelho']}{tabuleiro[i][0]}{Cores['reset']} │ {Cores['azul']}{tabuleiro[i][1]}{Cores['reset']} │ {Cores['vermelho']}{tabuleiro[i][2]}{Cores['reset']} │")
        if i < 2:
            print("├───┼───┼───┤")
    print("└───┴───┴───┘")
...
def jogar():
    ...
    while True:
        ...
        if verificar_vitoria(tabuleiro, jogador):
            os.system('notify-send "Parabéns! Você venceu! 🎉🎊"')  # Notificação no desktop
